...
class SetCriterion(nn.Module):
    ...
    """
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses, cuda):
    ...
    def loss_cardinality(self, outputs, targets, indices, num_boxes):
        ...
        # card_err = F.mse_loss(torch.tensor(card_pred.float(), device=self.device, dtype=torch.float64), tgt_lengths.float())
        # print(card_pred.float())
        # print("@@@@@")
        # print(tgt_lengths.float())
        # card_err = F.cross_entropy(torch.tensor(card_pred.float(), device=self.device, dtype=torch.float64), tgt_lengths.float())
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        # loss_bbox = F.mse_loss(torch.tensor(src_boxes, device=self.device, dtype=torch.float64), target_boxes, reduction='none')
        # loss = nn.MSELoss(reduction='none')
        # loss_bbox = loss(src_boxes, target_boxes)
        ...
        loss_ciou = 1 - torch.diag(box_ops.ciou_loss(
            box_ops.box_cxcywh_to_xyxy(src_boxes),
...
def build(args):
    ...
    ## num_classes 정의해주기
    num_classes = 20 if args.dataset_file != 'coco' else args.num_classes # coco_class = 20 + 1 (non object)
    ...
    criterion = SetCriterion(num_classes, matcher=matcher, weight_dict=weight_dict,
                             eos_coef=args.eos_coef, losses=losses, cuda=device)
    criterion.to(device)
