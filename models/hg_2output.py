...
class Hourglass_module_SB(torch.nn.Module):
    ...
    def __init__(self, input_ch, num_landmarks, hg_depth = 4, upsample_mode = 'nearest'):
        ...
        self.conv_final = nn.Conv2d(input_ch * 2, 1, 1)
    ...
    def forward(self, input):
        ...
        # conv_input = self.upsample_layer(conv_input)
...
if __name__ == '__main__':
    batch = 6
    dummy = torch.zeros((batch, 1, 128, 128)).to("cuda") #max_batch=608
    ...
    net = Single_hourglass_network(num_landmarks = 2, hg_depth=2).to("cuda")
    heat = net(dummy)
    print(heat.shape)
